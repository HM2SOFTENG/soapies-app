...
import {
  ...
  Check, ChevronRight, Sparkles, Mail, MessageSquare, Smartphone, AlertTriangle
} from "lucide-react";
...
export default function UserSettings() {
  ...
  const [deactivateStep, setDeactivateStep] = useState<"idle" | "confirm" | "code">("idle");
  const [deactivateCode, setDeactivateCode] = useState("");

  const requestDeactivation = trpc.auth.requestDeactivation.useMutation({
    onSuccess: () => { setDeactivateStep("code"); toast.info("Check your email for the confirmation code."); },
    onError: (e: any) => toast.error(e.message),
  });
      ...
      <div className="container px-4 py-6 sm:py-8 max-w-2xl mx-auto">
        ...
        >
          ...
          <motion.div
            ...
            className="glass-strong rounded-2xl border border-red-200/60 overflow-hidden shadow-md"
          >
            ...
            <div className="p-5">
              {deactivateStep === "idle" && (
                <div>
                  <p className="text-sm text-gray-600 mb-3">Deactivating your account will suspend your access. You can contact support to reactivate.</p>
                  <Button
                    variant="outline"
              ...
              {deactivateStep === "confirm" && (
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-red-700">Are you sure? This will suspend your account access.</p>
                  <p className="text-xs text-gray-500">We'll send a confirmation code to <strong>{user?.email}</strong>.</p>
                  <div className="flex gap-2">
                    <Button
                    ...
                    >
                      {requestDeactivation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Send Code"}
                    </Button>
                    <Button variant="outline" onClick={() => setDeactivateStep("idle")} className="rounded-xl border-gray-200">Cancel</Button>
                  </div>
                </div>
              )}
              {deactivateStep === "code" && (
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    placeholder="000000"
                    maxLength={6}
                    className="w-full px-4 py-2.5 rounded-xl border border-red-200 bg-white/70 text-sm outline-none focus:border-red-400 font-mono tracking-widest text-center"
                  />
                  <div className="flex gap-2">
                    <Button
                      onClick={() => confirmDeactivation.mutate({ code: deactivateCode })}
                      disabled={confirmDeactivation.isPending || deactivateCode.length < 4}
                      className="bg-red-500 hover:bg-red-600 text-white rounded-xl gap-2"
                    >
                      {confirmDeactivation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Confirm Deactivation"}
                    </Button>
                    <Button variant="outline" onClick={() => { setDeactivateStep("idle"); setDeactivateCode(""); }} className="rounded-xl border-gray-200">Cancel</Button>
                  </div>
                </div>
              )}
            </div>
