...
function StatChip({ label, value, tone, theme }: any) {
  const palette =
    tone === 'primary'
      ? {
          bg: theme.alpha(theme.colors.primary, 0.12),
          border: theme.alpha(theme.colors.primary, 0.28),
          text: theme.colors.primary,
        }
      : tone === 'warning'
        ? {
            bg: theme.alpha(theme.colors.warning, 0.12),
            border: theme.alpha(theme.colors.warning, 0.28),
            text: theme.colors.warning,
          }
        : tone === 'success'
          ? {
              bg: theme.alpha(theme.colors.success, 0.12),
              border: theme.alpha(theme.colors.success, 0.28),
  ...
  return (
    ...
    >
      ...
      <Text style={{ color: palette.text, fontSize: 13, fontWeight: '800', marginTop: 4 }}>
        {value || '—'}
      </Text>
    </View>
  );
  ...
  return (
    <View
      style={{
        ...
        borderWidth: 1,
        borderColor: theme.alpha(color, 0.24),
      }}
    >
  ...
  return (
    <TouchableOpacity
      ...
      style={{
        ...
        alignItems: 'center',
        gap: 12,
        backgroundColor: danger ? 'rgba(255,59,48,0.08)' : 'transparent',
        borderWidth: 1,
        borderColor: danger ? 'rgba(255,59,48,0.22)' : 'rgba(127,127,127,0.18)',
        borderRadius: 14,
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        {!!subtitle && (
          <Text style={{ color: color.subtle, fontSize: 12, marginTop: 2 }}>{subtitle}</Text>
        )}
      </View>
...
export default function AdminMembersScreen() {
  ...
  } = trpc.admin.adminMemberDetail.useQuery(
  ...
  const roleMutation = trpc.admin.updateUserRole.useMutation({
    onSuccess: () => {
      utils.admin.adminMembers.invalidate();
  ...
  const clearSignalMutation = trpc.admin.clearMemberSignal.useMutation({
    onSuccess: () => {
      utils.admin.adminMemberDetail.invalidate();
      Alert.alert('Updated', 'Member signal cleared.');
    },
    onError: (e: any) => Alert.alert('Error', e.message),
  });
  ...
  const suspendMutation = trpc.admin.suspendUser.useMutation({
    onSuccess: () => {
      utils.admin.adminMembers.invalidate();
      utils.admin.adminMemberDetail.invalidate();
      Alert.alert('Updated', 'Member suspended.');
    },
    onError: (e: any) => Alert.alert('Error', e.message),
  });
  const unsuspendMutation = trpc.admin.unsuspendUser.useMutation({
    onSuccess: () => {
      utils.admin.adminMembers.invalidate();
      utils.admin.adminMemberDetail.invalidate();
      Alert.alert('Updated', 'Member restored.');
    },
    onError: (e: any) => Alert.alert('Error', e.message),
  });
  ...
  const reviewProfileChangeMutation = trpc.admin.reviewMemberProfileChange.useMutation({
    onSuccess: () => {
      utils.admin.adminMemberDetail.invalidate();
      Alert.alert('Updated', 'Profile change request reviewed.');
    },
    onError: (e: any) => Alert.alert('Error', e.message),
  });
  const reviewGroupChangeMutation = trpc.admin.reviewMemberGroupChange.useMutation({
    onSuccess: () => {
      utils.admin.adminMemberDetail.invalidate();
      Alert.alert('Updated', 'Group change request reviewed.');
    },
    onError: (e: any) => Alert.alert('Error', e.message),
  });
  ...
  const reviewApplicationMutation = trpc.admin.reviewMemberApplication.useMutation({
    onSuccess: () => {
      utils.admin.adminMembers.invalidate();
      utils.admin.adminMemberDetail.invalidate();
      Alert.alert('Updated', 'Application reviewed.');
    },
    onError: (e: any) => Alert.alert('Error', e.message),
  });
  const advanceApplicationMutation = trpc.admin.advanceMemberApplication.useMutation({
    onSuccess: () => {
      utils.admin.adminMembers.invalidate();
      utils.admin.adminMemberDetail.invalidate();
      Alert.alert('Updated', 'Application phase updated.');
    },
    onError: (e: any) => Alert.alert('Error', e.message),
  });
  const confirmReservationMutation = trpc.admin.confirmMemberReservation.useMutation({
    onSuccess: () => {
      utils.admin.adminMembers.invalidate();
      utils.admin.adminMemberDetail.invalidate();
      Alert.alert('Updated', 'Payment confirmed and reservation updated.');
    },
    onError: (e: any) => Alert.alert('Error', e.message),
  });
  const reviewTestMutation = trpc.admin.reviewMemberTest.useMutation({
    onSuccess: () => {
      utils.admin.adminMembers.invalidate();
      utils.admin.adminMemberDetail.invalidate();
      Alert.alert('Updated', 'Test result reviewed.');
    },
    onError: (e: any) => Alert.alert('Error', e.message),
  });
  const setMemberGroupsMutation = trpc.admin.setMemberGroups.useMutation({
    onSuccess: (result: any) => {
      utils.admin.adminMemberDetail.invalidate();
      setSelectedGroupIds(
        (result as any)?.memberships?.map((membership: any) => membership.groupId) ?? []
      );
  ...
  const primaryAction = {
    bg: theme.alpha(theme.colors.primary, 0.14),
    fg: theme.colors.primary,
    text: theme.colors.text,
    subtle: theme.colors.textMuted,
    chevron: theme.colors.primary,
  };
  const warningAction = {
    bg: theme.alpha(theme.colors.warning, 0.14),
    fg: theme.colors.warning,
    text: theme.colors.text,
    subtle: theme.colors.textMuted,
    chevron: theme.colors.warning,
  };
  const purpleAction = {
    bg: theme.alpha(theme.colors.purple, 0.14),
    fg: theme.colors.purple,
    text: theme.colors.text,
    subtle: theme.colors.textMuted,
    chevron: theme.colors.purple,
  };
  const dangerAction = {
  ...
  return (
    ...
    >
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 60 }}>
        ...
        >
          ...
          >
            ...
            <TextInput
              ...
              style={{ flex: 1, color: theme.colors.text, paddingVertical: 12 }}
            />
          </View>
          <Text
            style={{
              ...
              return (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    backgroundColor: active
                      ? theme.alpha(theme.colors.secondary, 0.12)
                      : theme.colors.page,
                  }}
                >
                  <Text
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
            <TouchableOpacity
              ...
              style={{
                ...
                borderRadius: 12,
                borderWidth: 1,
                borderColor: groupFilter === 'all' ? theme.colors.primary : theme.colors.border,
                backgroundColor:
                  groupFilter === 'all'
                    ? theme.alpha(theme.colors.primary, 0.12)
                    : theme.colors.page,
            ...
            >
              <Text
                style={{
                  color: groupFilter === 'all' ? theme.colors.primary : theme.colors.textSecondary,
                  fontWeight: '700',
                  fontSize: 12,
                }}
              >
        ...
        {isLoading ? (
          <ActivityIndicator color={theme.colors.pink} style={{ marginTop: 40 }} />
        ) : members.length === 0 ? (
          <View style={{ alignItems: 'center', paddingTop: 60 }}>
            <Ionicons name="people-outline" size={42} color={theme.colors.textMuted} />
            <Text
              style={{ color: theme.colors.text, fontSize: 16, fontWeight: '700', marginTop: 12 }}
            >
              No members found
            </Text>
            <Text
            ...
            >
              Try clearing filters or check whether admin member data is available on this
              environment.
            </Text>
          </View>
        ) : (
          members.map((member: any) => (
            <TouchableOpacity
              ...
              style={{
                ...
                padding: 14,
                borderWidth: 1,
                borderColor: member.alerts?.total
                  ? theme.alpha(theme.colors.warning, 0.28)
                  : theme.colors.border,
                marginBottom: 10,
              }}
            >
                ...
                <View style={{ flex: 1 }}>
                    ...
                    {member.alerts?.total ? (
                      <View
                        style={{
                          ...
                          paddingVertical: 4,
                          borderRadius: 999,
                          backgroundColor: theme.alpha(theme.colors.warning, 0.14),
                          borderWidth: 1,
                          borderColor: theme.alpha(theme.colors.warning, 0.28),
                        }}
                      >
                        <Text
                          style={{ color: theme.colors.warning, fontSize: 11, fontWeight: '800' }}
                        >
                          {member.alerts.total} alert{member.alerts.total === 1 ? '' : 's'}
                        </Text>
                      </View>
                    ) : null}
                  </View>
                  <Text style={{ color: theme.colors.textMuted, fontSize: 12, marginTop: 2 }}>
                    {member.email || member.phone || `User #${member.userId}`}
                  </Text>
                  <Text style={{ color: theme.colors.textSecondary, fontSize: 12, marginTop: 4 }}>
                    {[
                      formatLabel(member.memberRole || member.role),
                      formatLabel(member.applicationStatus),
                      member.communityId,
                    ]
                      .filter(Boolean)
                      .join(' • ')}
                  ...
                  {member.alerts?.total ? (
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
                      <AlertBadge
                        label="Application"
                        count={member.alerts.application}
                        color={theme.colors.warning}
                        theme={theme}
                      />
                      <AlertBadge
                        label="Profile"
                        count={member.alerts.profileChanges}
                        color={theme.colors.primary}
                        theme={theme}
                      />
                      <AlertBadge
                        label="Group"
                        count={member.alerts.groupChanges}
                        color={theme.colors.secondary}
                        theme={theme}
                      />
                      <AlertBadge
                        label="Payments"
                        count={member.alerts.payments}
                        color={theme.colors.purple}
                        theme={theme}
                      />
                      <AlertBadge
                        label="Testing"
                        count={member.alerts.tests}
      ...
      >
        ...
        >
          ...
          >
            ...
            <Text
              numberOfLines={1}
              style={{ color: theme.colors.text, fontSize: 20, fontWeight: '800', flex: 1 }}
            >
              Member Overview
            </Text>
          </View>
          <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 60 }}>
            {detailLoading ? (
              ...
              <View style={{ alignItems: 'center', paddingTop: 48, paddingHorizontal: 24 }}>
                ...
                >
                  {(detailError as any)?.message ?? 'Please try again in a moment.'}
                </Text>
                <TouchableOpacity
              ...
              <>
                ...
                >
                  <View>
                    <Text style={{ color: theme.colors.text, fontSize: 24, fontWeight: '900' }}>
                      {selectedProfile?.displayName || selectedUser?.name || 'Member'}
                    </Text>
                    <Text style={{ color: theme.colors.textMuted, marginTop: 4 }}>
                      {selectedUser?.email || selectedUser?.phone || `User #${selectedUser?.id}`}
                    </Text>
                    <Text style={{ color: theme.colors.textSecondary, marginTop: 8 }}>
                      {[
                        formatLabel(selectedProfile?.applicationStatus),
                        formatLabel(selectedProfile?.memberRole || selectedUser?.role),
                        selectedProfile?.communityId,
                      ]
                        .filter(Boolean)
                        .join(' • ')}
                    </Text>
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                    <StatChip
                      label="Role"
                      value={formatLabel(selectedProfile?.memberRole || selectedUser?.role)}
                      tone="primary"
                      theme={theme}
                    />
                    <StatChip
                      label="Status"
                      value={formatLabel(selectedProfile?.applicationStatus)}
                      tone={
                        selectedProfile?.applicationStatus === 'approved'
                          ? 'success'
                          : selectedProfile?.applicationStatus === 'waitlisted'
                            ? 'warning'
                            : 'neutral'
                      }
                      theme={theme}
                    ...
                    <StatChip
                      label="Credits"
                      value={String(selectedCredits.length)}
                      tone="neutral"
                      theme={theme}
                    />
                    <StatChip
                      label="Referrals"
                      value={String(selectedReferralCodes.length)}
                      tone="neutral"
                      theme={theme}
                    />
                    <StatChip
                ...
                >
                  ...
                  {pendingProfileRequests.length === 0 && pendingGroupRequests.length === 0 ? (
                    <Text style={{ color: theme.colors.textMuted, fontSize: 13 }}>
                      No pending profile or group requests for this member.
                    </Text>
                  ) : null}
                    ...
                    >
                      <View>
                        <Text style={{ color: theme.colors.text, fontSize: 14, fontWeight: '800' }}>
                          Profile change: {formatLabel(request.fieldName)}
                        </Text>
                        <Text
                      ...
                      <View style={{ flexDirection: 'row', gap: 10 }}>
                        <TouchableOpacity
                          onPress={() =>
                            reviewProfileChangeMutation.mutate({
                              id: request.id,
                              status: 'approved',
                            })
                          }
                          style={{
                            flex: 1,
                            backgroundColor: theme.alpha(theme.colors.success, 0.14),
                            borderWidth: 1,
                        ...
                        <TouchableOpacity
                          onPress={() =>
                            reviewProfileChangeMutation.mutate({ id: request.id, status: 'denied' })
                          }
                          style={{
                            flex: 1,
                            backgroundColor: theme.alpha(theme.colors.warning, 0.14),
                            borderWidth: 1,
                            borderColor: theme.alpha(theme.colors.warning, 0.3),
                            borderRadius: 12,
                            paddingVertical: 10,
                            alignItems: 'center',
                          }}
                  ...
                  {pendingGroupRequests.map((request: any) => {
                    const requestedGroup = availableGroups.find(
                      (group: any) => group.id === request.requestedGroupId
                    );
                    const currentGroup = availableGroups.find(
                      (group: any) => group.id === request.currentGroupId
                    );
                    return (
                      <View
                      ...
                      >
                        <View>
                          <Text
                            style={{ color: theme.colors.text, fontSize: 14, fontWeight: '800' }}
                          >
                          ...
                          >
                            Requested:{' '}
                            {requestedGroup?.name || `Group #${request.requestedGroupId}`}
                          </Text>
                          {request.reason ? (
                            <Text
                              style={{ color: theme.colors.textMuted, fontSize: 12, marginTop: 6 }}
                            >
                        ...
                        <View style={{ flexDirection: 'row', gap: 10 }}>
                          <TouchableOpacity
                            onPress={() =>
                              reviewGroupChangeMutation.mutate({
                                id: request.id,
                                status: 'approved',
                              })
                            }
                          ...
                          <TouchableOpacity
                            onPress={() =>
                              reviewGroupChangeMutation.mutate({ id: request.id, status: 'denied' })
                            }
                            style={{
                              flex: 1,
                              backgroundColor: theme.alpha(theme.colors.warning, 0.14),
                ...
                >
                  ...
                  <Text style={{ color: theme.colors.textMuted, fontSize: 13 }}>
                    Assign one or more community groups directly for this member.
                  </Text>
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      setMemberGroupsMutation.mutate({
                        userId: selectedUserId!,
                    ...
                    style={{
                      ...
                      borderWidth: 1,
                      borderColor: theme.alpha(theme.colors.primary, 0.3),
                      borderRadius: 12,
                      paddingHorizontal: 14,
                      paddingVertical: 10,
                    }}
                ...
                >
                  ...
                  pendingTests.length === 0 ? (
                    <Text style={{ color: theme.colors.textMuted, fontSize: 13 }}>
                      No pending application, payment, or testing actions for this member.
                    </Text>
                  ) : null}
                    ...
                    >
                      <View>
                        ...
                        <Text
                          style={{ color: theme.colors.textSecondary, fontSize: 12, marginTop: 4 }}
                        >
                          Phase: {formatLabel(item.applicationPhase || 'initial_review')} · Status:{' '}
                          {formatLabel(item.applicationStatus || 'submitted')}
                        </Text>
                      </View>
                      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                        <TouchableOpacity
                        ...
                        <TouchableOpacity
                          ...
                          style={{
                            backgroundColor: theme.alpha('#ff3b30', 0.14),
                            borderWidth: 1,
                            borderColor: theme.alpha('#ff3b30', 0.3),
                            borderRadius: 12,
                            paddingHorizontal: 12,
                            paddingVertical: 10,
                          }}
                        >
                          <Text style={{ color: '#ff3b30', fontWeight: '800' }}>Reject</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          onPress={() =>
                            advanceApplicationMutation.mutate({
                              profileId: item.id,
                    ...
                    >
                      <View>
                        ...
                        <Text
                          style={{ color: theme.colors.textSecondary, fontSize: 12, marginTop: 4 }}
                        >
                          {reservation.eventTitle || reservation.event?.title || 'Reservation'} ·{' '}
                          {formatLabel(reservation.ticketType)} ·{' '}
                          {formatLabel(reservation.paymentMethod)}
                        </Text>
                      </View>
                      <TouchableOpacity
                        onPress={() =>
                          confirmReservationMutation.mutate({ reservationId: reservation.id })
                        }
                        style={{
                          alignSelf: 'flex-start',
                          backgroundColor: theme.alpha(theme.colors.success, 0.14),
                  ...
                  {pendingTests.map((test: any) => (
                    <View
                      key={`test-${test.id || test.submission?.id}`}
                      style={{
                        backgroundColor: theme.colors.page,
                        borderWidth: 1,
                        borderColor: theme.colors.border,
                    ...
                    >
                      <View>
                        ...
                        <Text
                          style={{ color: theme.colors.textSecondary, fontSize: 12, marginTop: 4 }}
                        >
                          {test.event?.title || 'Event pending review'}
                        </Text>
                      </View>
                      <View style={{ flexDirection: 'row', gap: 10 }}>
                        <TouchableOpacity
                ...
                >
                  ...
                  <ActionRow
                    icon="shield-outline"
                    title="Change Role"
                    subtitle="Promote, demote, or grant admin access"
                    color={primaryAction}
                    onPress={() =>
                      Alert.alert('Update Role', 'Choose role', [
                        {
                          text: formatLabel('member')!,
                          onPress: () =>
                            roleMutation.mutate({
                              userId: selectedUserId!,
                        ...
                        {
                          text: formatLabel('angel')!,
                          onPress: () =>
                            roleMutation.mutate({
                              userId: selectedUserId!,
                              memberRole: 'angel' as any,
                            }),
                        },
                        {
                          text: formatLabel('admin')!,
                          onPress: () =>
                            roleMutation.mutate({
                              userId: selectedUserId!,
                  ...
                  <ActionRow
                    ...
                    title="Clear Signal"
                    subtitle="Reset current meetup availability signal"
                    color={warningAction}
                    onPress={() => clearSignalMutation.mutate({ userId: selectedUserId! })}
                  />
                  <ActionRow
                    icon="notifications-outline"
                    title="Send Nudge"
                    subtitle="Send a lightweight admin check-in notification"
                    color={purpleAction}
                    onPress={() =>
                      nudgeMutation.mutate({
                        userId: selectedUserId!,
                        title: 'Admin Check-In',
                        body: 'Hi — the team wanted to check in with you. Reply in app if you need anything.',
                      })
                    }
                  />
                  <ActionRow
                    ...
                    subtitle="Email a password reset code to this member"
                    color={primaryAction}
                    onPress={() => requestPasswordResetMutation.mutate({ userId: selectedUserId! })}
                  />
                  {!isSuspended ? (
                    <ActionRow
                      icon="pause-circle-outline"
                      title="Suspend Member"
                      subtitle="Temporarily disable access without deleting data"
                      color={warningAction}
                      onPress={() =>
                        Alert.alert('Suspend member?', 'This will suspend access until restored.', [
                          { text: 'Cancel', style: 'cancel' },
                          {
                            text: 'Suspend',
                            style: 'destructive',
                            onPress: () => suspendMutation.mutate({ userId: selectedUserId! }),
                          },
                        ])
                      }
                    />
                    ...
                    <ActionRow
                      ...
                      subtitle="Re-enable access for this member"
                      color={primaryAction}
                      onPress={() => unsuspendMutation.mutate({ userId: selectedUserId! })}
                    />
                  )}
                  {selectedProfile?.userId ? (
                    <ActionRow
                      ...
                      subtitle="Open the member-facing profile view"
                      color={neutralAction}
                      onPress={() => {
                        const targetUserId = selectedProfile.userId;
                        const returnUserId = selectedUserId;
                        setSelectedUserId(null);
                        setTimeout(
                          () =>
                            router.push({
                              pathname: '/member/[id]' as any,
                              params: {
                                id: String(targetUserId),
                                returnTo: 'admin-members',
                                reopenUserId: String(returnUserId),
                              },
                            } as any),
                          150
                        );
                ...
                >
                  ...
                  <Text style={{ color: theme.colors.textSecondary, fontSize: 13, lineHeight: 19 }}>
                    Use suspend for reversible access control, clear signal to remove current
                    casual-meetup intent, and delete only for permanent removal after review. For
                    application, payment, and testing items, triage from the member card alert
                    badges until those direct admin actions are added here.
                  </Text>
                  <ActionRow
                    icon="trash-outline"
                    title="Delete Member"
                    subtitle="Permanent removal and related cleanup"
                    color={dangerAction}
                    danger
                    onPress={() =>
                      Alert.alert(
                        'Delete member permanently?',
                        'This action is destructive and removes related user data.',
                        [
                          { text: 'Cancel', style: 'cancel' },
                          {
                            text: 'Delete',
                            style: 'destructive',
                            onPress: () => deleteMutation.mutate({ userId: selectedUserId! }),
                          },
                        ]
                      )
                    }
