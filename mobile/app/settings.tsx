  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        {subtitle && (
          <Text style={{ color: themeColors.textMuted, fontSize: 12, marginTop: 2 }}>
            {subtitle}
          </Text>
        )}
      </View>
...
export default function SettingsScreen() {
  ...
  const {
    ...
    error: meError,
    refetch: refetchMe,
  } = trpc.auth.me.useQuery(undefined, { enabled: hasToken, staleTime: 60_000 });
  ...
  useEffect(() => {
    (async () => {
      ...
      if (map[PRIVACY_MESSAGING_KEY])
        setMessagingPref((map[PRIVACY_MESSAGING_KEY] as MessagingOption) ?? 'everyone');
    })();
  }, []);
            ...
            Alert.alert(
              ...
              [
                { text: 'Cancel', style: 'cancel' },
                {
                  text: 'Yes, Delete',
                  style: 'destructive',
                  onPress: () => deleteMeMutation.mutate(),
                },
              ]
            );
          },
  ...
  function handleRateApp() {
    if (!APP_STORE_ID) {
      Alert.alert(
        'Coming Soon',
        'Rating will be available when the app launches on the App Store.'
      );
      return;
    }
    Linking.openURL(`https://apps.apple.com/app/id${APP_STORE_ID}?action=write-review`).catch(
      () => {
        Alert.alert(
          ...
          'Please find Soapies on the App Store to leave a review.'
        );
      }
    );
  }
    ...
    <ThemedScreen scroll contentContainerStyle={{ paddingBottom: 120 }}>
      ...
      <View style={{ paddingTop: insets.top + 12, paddingBottom: 18, paddingHorizontal: 20 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <Text style={{ color: themeColors.text, fontSize: 24, fontWeight: '900', flex: 1 }}>
            Settings ⚙️
          </Text>
        </View>
      </View>
      ...
      {settingsLoadError && (
        <View
          style={{
            marginHorizontal: 16,
            marginBottom: 12,
            backgroundColor: alpha(themeColors.danger ?? '#ff3b30', 0.08),
            borderRadius: 14,
            borderWidth: 1,
            borderColor: alpha(themeColors.danger ?? '#ff3b30', 0.18),
            padding: 14,
          }}
        >
          <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ color: themeColors.text, fontWeight: '800', fontSize: 14 }}>
                Some settings data could not load
              </Text>
              <Text
                style={{
              ...
              >
                {settingsLoadErrorMessage ??
                  'Retry to refresh your account details and connection counts.'}
              </Text>
            </View>
            <TouchableOpacity
              onPress={() => {
            ...
            >
              <Text style={{ color: themeColors.text, fontWeight: '700', fontSize: 12 }}>
                Retry
              </Text>
            </TouchableOpacity>
          </View>
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ color: themeColors.textMuted, fontSize: 12, marginTop: 1 }}>
      ...
      <ChevronRow
        ...
        onPress={() =>
          router.push({ pathname: '/(auth)/forgot-password' as any, params: { prefill: email } })
        }
      />
      <ChevronRow
        icon="mail-outline"
        ...
        onPress={() =>
          Alert.alert(
            'Contact Change',
            'To change your email or phone number, please contact support at support@soapies.app.'
          )
        }
      />
      <ChevronRow
      ...
      <ToggleRow
        ...
        onToggle={(v) => {
          setShowPulseLoc(v);
          saveBool(PRIVACY_PULSE_LOC_KEY, v);
        }}
      />
