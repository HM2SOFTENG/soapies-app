...
function BadgeIcon({ name, color, size, badge, focused }: { name: any; color: string; size: number; badge?: number; focused?: boolean }) {
  ...
  return (
    <Animated.View style={[styles.iconWrap, focused && themedFocusedWrap(colors), { transform: [{ scale }, { translateY }] }]}>
...
function AdminBadgeIcon({ name, color, size, focused }: { name: any; color: string; size: number; focused?: boolean }) {
  ...
  const scale = scaleAnim.interpolate({ inputRange: [0, 1], outputRange: [1, 1.08] });
  return (
    <Animated.View style={[styles.iconWrap, focused && themedFocusedWrap(colors), { transform: [{ scale }] }]}>
      <Ionicons name={name} size={size} color={color} />
...
function PulseTabIcon({ color, size, focused, signalType }: { color: string; size: number; focused: boolean; signalType: string | null }) {
  ...
  return (
    <View style={[styles.iconWrap, (focused || isActive) && themedFocusedWrap(colors)]}>
      ...
      {isActive && (
        <View style={{ position: 'absolute', top: 2, right: 2, width: 8, height: 8, borderRadius: 4, backgroundColor: sigColor, borderWidth: 2, borderColor: colors.tabBar }} />
      )}
    </View>
  );
}
...
function TabBarIcon({ name, color, size, focused }: { name: any; color: string; size: number; focused: boolean }) {
  ...
  return (
    <Animated.View style={[styles.iconWrap, focused && themedFocusedWrap(colors), { transform: [{ scale }, { translateY }] }]}>
      {focused && <Animated.View style={[styles.iconShimmer, { backgroundColor: isDark ? 'rgba(255,255,255,0.16)' : 'rgba(236,72,153,0.10)', opacity: shimmerOpacity, transform: [{ scale: shimmerScale }] }]} />}
      <Ionicons name={name} size={size} color={color} />
      <Animated.View style={{ width: 4, height: 4, borderRadius: 2, backgroundColor: colors.primary, opacity: dotOpacity, marginTop: 4 }} />
    </Animated.View>
  );
}
